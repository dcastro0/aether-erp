...
WHERE organization_id = $1
  ...
  AND (sqlc.narg('start_date')::date IS NULL OR due_date >= sqlc.narg('start_date'))
  AND (sqlc.narg('end_date')::date IS NULL OR due_date <= sqlc.narg('end_date'))
...
SELECT
    ...
    COALESCE(SUM(CASE WHEN type = 'payable' AND status = 'paid' THEN amount ELSE 0 END), 0)::decimal AS total_payable_paid,
    COALESCE(SUM(CASE WHEN type = 'receivable' AND status = 'paid' THEN amount ELSE 0 END), 0)::decimal AS total_receivable_paid
